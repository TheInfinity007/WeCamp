...
router.get('/', (req, res)=>{
	...
	if(req.query.search){
		...
		Campground.find({name: regex}).skip((perPage*pageQuery)- perPage).limit(perPage).sort({"createdAt": -1}).exec((err, allCampgrounds)=>{
		...
		Campground.find({}).skip((perPage*pageQuery)-perPage).limit(perPage).sort({"createdAt": -1}).exec((err, allCampgrounds)=>{
